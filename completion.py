...
def align_output_to_input(mesh_path, best_elev, best_azim, pcd_center, pcd_scale):
    """
    mesh_path: Path to the .glb/.obj generated by SPAR3D
    best_elev: Elevation found during canonical search (degrees)
    best_azim: Azimuth found during canonical search (degrees)
    pcd_center: The original center of your partial point cloud (from pcd.get_center())
    pcd_scale: The max_extent/radius used to "fake" normalization
    """
    # 1. Load the mesh
    mesh = o3d.io.read_triangle_mesh(mesh_path)
    
    # 2. Scale it back up
    # SPAR3D outputs are normalized (usually radius 1.0). 
    # We multiply by your saved max_radius to match the Redwood scale.
    mesh.scale(pcd_scale, center=(0, 0, 0))

    # 3. Create Rotation Matrices
    # Note: We use the positive angles because we are moving the mesh 
    # FROM the camera's front view BACK to the world position.
    ...
    # Depending on your coordinate system, you may need -np.radians
    rad_azim = np.radians(best_azim)
    R_azim = mesh.get_rotation_matrix_from_axis_angle([0, -rad_azim, 0])
    
    # 4. Apply Rotations (Order: Elevation then Azimuth)
    # This matches the way a 'look-at' camera is constructed
    combined_R = R_azim @ R_elev
    mesh.rotate(combined_R, center=(0, 0, 0))

    # 5. Translate to original world position
